...
impl App<'_> {
    pub fn render_title(&self) -> impl Widget {
        ...
        let widget = Paragraph::new(
            Text::styled("Create New Json", Style::default().fg(Color::Green)).centered(),
        )
        .block(widget_style);
    ...
    pub fn render_edition(&self) -> impl Widget {
        let widget_style = Block::default()
            .borders(Borders::ALL)
            .style(Style::default());

        let widget = match self.ref_edition.is_empty() {
            false => {
                let str_json = "".to_string();
                Paragraph::new(Text::styled(str_json, Style::default().fg(Color::Green)))
            }
            true => Paragraph::new(Text::styled(
                "No value selected",
    ...
    pub fn ui(&self, frame: &mut Frame) {
        ...
        let widget_style = Block::default()
            .borders(Borders::ALL)
            .style(Style::default());
        let lines = self.json_container.render();
        let json_para = Paragraph::new(lines).block(widget_style);

        frame.render_widget(json_para, screens[0]);
